...
export const Grammar: $.Parser<$ast.Grammar> = $.loc($.field($.pure("Grammar"), "$", $.field($.star($.ref(() => Rule)), "rules", $.eps)));
export const Rule: $.Parser<$ast.Rule> = $.loc($.field($.pure("Rule"), "$", $.field($.ref(() => ident), "name", $.field($.opt($.ref(() => $string)), "display", $.field($.opt($.ref(() => formals)), "formals", $.right($.str("="), $.field($.ref(() => ruleBody), "body", $.right($.str(";"), $.eps))))))));
...
export const Alt: $.Parser<$ast.Alt> = $.loc($.field($.pure("Alt"), "$", $.field($.ref(() => Seq), "head", $.field($.star($.right($.str("/"), $.ref(() => Seq))), "tail", $.eps))));
export const ruleBody: $.Parser<$ast.ruleBody> = Alt;
export const Seq: $.Parser<$ast.Seq> = $.loc($.field($.pure("Seq"), "$", $.field($.star($.ref(() => Sel)), "exprs", $.eps)));
export const Sel: $.Parser<$ast.Sel> = $.loc($.field($.pure("Sel"), "$", $.field($.opt($.ref(() => selector)), "selector", $.field($.ref(() => Iter), "expr", $.eps))));
...
export const Iter: $.Parser<$ast.Iter> = $.loc($.field($.pure("Iter"), "$", $.field($.star($.regex<"!" | "&" | "#" | "$">("!&#$", [$.ExpString("!"), $.ExpString("&"), $.ExpString("#"), $.ExpString("$")])), "prefix", $.field($.ref(() => base), "expr", $.field($.opt($.regex<"*" | "+" | "?">("*+?", [$.ExpString("*"), $.ExpString("+"), $.ExpString("?")])), "suffix", $.eps)))));
export const Apply: $.Parser<$ast.Apply> = $.loc($.field($.pure("Apply"), "$", $.field(ident, "name", $.field($.opt($.ref(() => params)), "params", $.eps))));
export const Terminal: $.Parser<$ast.Terminal> = $.loc($.field($.pure("Terminal"), "$", $.field($.ref(() => $string), "value", $.eps)));
...
export const base: $.Parser<$ast.base> = $.alt(Apply, $.alt($.lex($.ref(() => Class)), $.alt(Terminal, $.alt(paren, Any))));
export const params: $.Parser<$ast.params> = $.right($.str("<"), $.left($.opt($.field(Alt, "head", $.field($.star($.right($.str(","), Alt)), "tail", $.eps))), $.str(">")));
export const Class: $.Parser<$ast.Class> = $.loc($.field($.pure("Class"), "$", $.right($.str("["), $.field($.opt($.str("^")), "negated", $.field($.star($.ref(() => classSeq)), "seqs", $.right($.str("]"), $.eps))))));
export const Group: $.Parser<$ast.Group> = $.loc($.field($.pure("Group"), "$", $.field($.ref(() => classChar), "from", $.right($.str("-"), $.field($.ref(() => classChar), "to", $.eps)))));
...
export const Short: $.Parser<$ast.Short> = $.loc($.field($.pure("Short"), "$", $.field($.stry($.right($.ref(() => hexDigit), $.right($.ref(() => hexDigit), $.right($.ref(() => hexDigit), $.right($.ref(() => hexDigit), $.eps))))), "value", $.eps)));
export const Long: $.Parser<$ast.Long> = $.loc($.field($.pure("Long"), "$", $.right($.str("{"), $.field($.stry($.plus($.ref(() => hexDigit))), "value", $.right($.str("}"), $.eps)))));
export const unicode: $.Parser<$ast.unicode> = $.right($.str("u"), $.alt(Short, Long));
export const Ascii: $.Parser<$ast.Ascii> = $.loc($.field($.pure("Ascii"), "$", $.right($.str("x"), $.field($.stry($.right($.ref(() => hexDigit), $.right($.ref(() => hexDigit), $.eps))), "value", $.eps))));
...
export const $string: $.Parser<$ast.$string> = $.lex($.right($.str("\""), $.left($.star($.ref(() => $char)), $.str("\""))));
